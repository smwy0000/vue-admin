...
<script>
import sha1 from 'js-sha1';//加密方式：sha1 md5
...
//   setup(props,context){
  ...
  setup(props,{refs,root }){
    ...
    const rules = ({
        ...
        /**
         * 1.不建议在一个方法里做多件不同的事件（尽可能只做自己本身的事情，不做其他人的事）
         * 2.尽量把相同的事情封装到一个方法里面，通过调用函数执行
         */
...
1.在前端预先加密一次
登录的密码：12345（普通字符串）
经过加密后：sha1('12345') == 'bdkjabfvbkue8327'（加密后的字符串）

2.后台加密
接收到的字符串：'bdkjabfvbkue8327'
后台再次加密：md5('bdkjabfvbkue8327') == 'ygytftfjguhjko87656789'（最终加密后的密码）
最终新的字符串写入数据库：ygytftfjguhjko87656789
